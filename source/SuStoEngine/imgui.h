...
typedef unsigned int ImGuiID;       // unique ID used by widgets (typically hashed from a stack ofstd::string)
...
namespace ImGui
{
    ...
    // Widgets: Text
    IMGUI_API void          TextUnformatted(const char* text, const char* text_end = NULL);               // doesn't require null terminatedstd::string if 'text_end' is specified. no copy done, no limits, recommended for long chunks of text
...
struct ImGuiIO
{
    ...
    IMGUI_API void AddInputCharactersUTF8(const char* utf8_chars);      // Add new characters into InputCharacters[] from an UTF-8std::string
...
struct ImFontAtlas
{
    ...
    // NB: Make sure that yourstd::string are UTF-8 and NOT in your local code page. In C++11, you can create UTF-8std::string literal using the u8"Hello world" syntax. See FAQ for details.
    ...
    // Helpers to build glyph ranges from text data. Feed your applicationstd::strings/characters to it then call BuildRanges().
    struct GlyphRangesBuilder
    {
        ...
        IMGUI_API void AddText(const char* text, const char* text_end = NULL);      // Addstd::string (each character of the UTF-8std::string are added)
