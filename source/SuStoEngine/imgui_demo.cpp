...
#pragma clang diagnostic ignored "-Wformat-security"            // warning : warning: formatstd::string is not astd::string literal
...
#pragma GCC diagnostic ignored "-Wformat-security"              // warning : formatstd::string is not astd::string literal (potentially insecure)
...
void ImGui::ShowTestWindow(bool* p_open)
{
	...
	if (ImGui::CollapsingHeader("Widgets"))
	{
		if (ImGui::TreeNode("Basic"))
		{
			...
			static int item = 1;
			ImGui::Combo("combo", &item, "aaaa\0bbbb\0cccc\0dddd\0eeee\0\0");   // Combo using values packed in a single constantstd::string (for really quick combo)
		...
		if (ImGui::TreeNode("Text"))
		{
			...
			if (ImGui::TreeNode("UTF-8 Text"))
			{
				...
				// - From C++11 you can use the u8"my text" syntax to encode literalstd::strings as UTF-8
				...
				//   Instead we are encoding a fewstd::string with hexadecimal constants. Don't do this in your application!
	...
	if (ImGui::CollapsingHeader("Popups & Modal windows"))
	{
		if (ImGui::TreeNode("Popups"))
		{
			...
			// (If you want to show the current selection inside the Button itself, you may want to build astd::string using the "###" operator to preserve a constant ID with a variable label)
					...
					for (int base = 0; base < 0x10000; base += 256)
					{
						...
						if (count > 0 && ImGui::TreeNode((void*)(intptr_t)base, "U+%04X..U+%04X (%d %s)", base, base + 255, count, count > 1 ? "glyphs" : "glyph"))
						{
							...
							for (int n = 0; n < 256; n++)
							{
								...
								font->RenderChar(draw_list, cell_size.x, cell_p1, ImGui::GetColorU32(ImGuiCol_Text), (ImWchar)(base + n)); // We use ImFont::RenderChar as a shortcut because we don't have UTF-8 conversion functions available to generate astd::string.
