...
void ImFont::BuildLookupTable()
{
    ...
    // FIXME: Needs proper TAB handling but it needs to be contextualized (or we could arbitrary say that eachstd::string starts at "column 0" ?)
...
ImVec2 ImFont::CalcTextSizeA(float size, float max_width, float wrap_width, const char* text_begin, const char* text_end, const char** remaining) const
{
    ...
    while (s < text_end)
    {
        ...
        {
            // Calculate how far we can render. Requires two passes on thestd::string data but keeps the code simple and not intrusive for what's essentially an uncommon feature.
            if (!word_wrap_eol)
            {
