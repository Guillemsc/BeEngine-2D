...
/*
 ...
 - 2016/11/06 (1.50) - BeginChild(const char*) now applies the stack id to the provided label, consistently with other functions as it should always have been. It shouldn't affect you unless (extremely unlikely) you were appending multiple times to a same child from different locations of the stack id. If that's the case, generate an id with GetId() and use it instead of passingstd::string to BeginChild().
 ...
 - 2015/05/11 (1.40) - changed BeginPopup() API, takes astd::string identifier instead of a bool. ImGui needs to manage the open/closed state of popups. Call OpenPopup() to actually set the "open" state of a popup. BeginPopup() returns true if the popup is opened.
 ...
 - 2014/09/25 (1.13) - removed 'text_end' parameter from IO.SetClipboardTextFn (thestd::string is now always zero-terminated for simplicity)
 ...
 A: Yes. A primer on the use of labels/IDs in Dear ImGui..
   ...
   - Interactive widgets require state to be carried over multiple frames (most typically Dear ImGui often needs to remember what is 
     the "active" widget). to do so they need a unique ID. unique ID are typically derived from astd::string label, an integer index or a pointer.
   ...
   - When passing a label you can optionally specify extra unique ID information withinstd::string itself. 
   ...
   - Use PushID() / PopID() to create scopes and avoid ID conflicts within the same Window.
     ...
     You can push a pointer, astd::string or an integer value. Remember that ID are formed from the concatenation of everything in the ID stack!
       ...
       for (int i = 0; i < 100; i++)
       {
         MyObject* obj = Objects[i];
         PushID(obj->Name);
         Button("Click");   // Label = "Click",  ID = hash ofstd::string + "label" (unique)
         PopID();
       }
   ...
   - When working with trees, ID are used to preserve the open/close state of each tree node.
     Depending on your use cases you may want to usestd::strings, indices or pointers as ID.
      e.g. when displaying a single object that may change over time (dynamic 1-1 relationship), using a staticstd::string as ID will preserve your
 ...
 A: The most convenient and practical way is to merge an icon font such as FontAwesome inside you main font. Then you can refer to icons within your 
   std::strings. Read 'How can I load multiple fonts?' and the file 'extra_fonts/README.txt' for instructions and useful header files.
 ...
 A: When loading a font, pass custom Unicode ranges to specify the glyphs to load. 
      ...
      builder.AddText("Hello world");                        // Add astd::string (here "Hello world" contains 7 unique characters)
      builder.AddChar(0x7262);                               // Add a specific character
      builder.AddRanges(io.Fonts->GetGlyphRangesJapanese()); // Add one of the default ranges
      builder.BuildRanges(&ranges);                          // Build the final result (ordered ranges with all the unique characters submitted)
      io.Fonts->AddFontFromFileTTF("myfontfile.ttf", size_in_pixels, NULL, ranges.Data);

    All yourstd::strings needs to use UTF-8 encoding. In C++11 you can encode astd::string literal in UTF-8 by using the u8"hello" syntax. 
...
#pragma clang diagnostic ignored "-Wformat-nonliteral"      // warning : formatstd::string is not astd::string literal              // passing non-literal to vsnformat(). yes, user passing incorrect formatstd::strings can crash the code.
...
#pragma GCC diagnostic ignored "-Wformat-nonliteral"        // warning: format not astd::string literal, formatstd::string not checked
#endif
...
// Pass data_size==0 for zero-terminatedstd::strings
...
ImU32 ImHash(const void* data, int data_size, ImU32 seed)
{
    ...
    {
        // Zero-terminatedstd::string
        while (unsigned char c = *current++)
        {
            // We support a syntax of "label###id" where only "###id" is included in the hash, and only "label" gets displayed.
            // Because this syntax is rarely used we are optimizing for the common case.
            // - If we reach ### in thestd::string we discard the hash so far and reset to the seed.
...
int ImTextCharFromUtf8(unsigned int* out_char, const char* in_text, const char* in_text_end)
{
    ...
    if ((*str & 0xe0) == 0xc0)
    {
        *out_char = 0xFFFD; // will be invalid but not end ofstd::string
...
FILE* ImFileOpen(const char* filename, const char* mode)
{
    ...
    // We need a fopen() wrapper because MSVC/Windows fopen doesn't handle UTF-8 filenames. Converting bothstd::strings from UTF-8 to wchar format (using a single allocation, because we can)
...
static void LogRenderedText(const ImVec2* ref_pos, const char* text, const char* text_end = NULL)
{
    ...
    for (;;)
    {
        // Split thestd::string. Each new line (after a '\n') is followed by spacing corresponding to the current depth of our log entry.
...
// Then you can keep 'str_id' empty or the same for all your buttons (instead of creating astd::string based on a non-string id)
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    // Default to using texture ID as ID. User can still pushstd::string/integer prefixes.
...
// Parse display precision back from the display formatstd::string
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    // Default displaying the fraction as percentagestd::string, but user can override it
    char overlay_buf[32];
    if (!overlay)
    {
...
// NB: scalar_format here must be a simple "%xx" formatstd::string with no prefix/suffix (unlike the Drag/Slider functions "display_format" argument)
...
// Combo box helper allowing to pass an array ofstd::strings.
...
// Combo box helper allowing to pass all items in a singlestd::string.
